...
\subsection{Driving Forces of Droplet Breakup}
...
- Along this interface, at $\approx 135 \degree$ from the horizontal axis, the accumulated shear switches from negative to positive. This area is highlighted in Figure~\ref{fig:shear_pressure_accumulation}
...
\begin{figure}
    ...
    \begin{subfigure}[b]{\textwidth}
        ...
        \caption{Accumulation of shear stress (top) and pressure (bottom)}
        \label{subfig:shear_pressure_accumulation_zoomed_out}
    \end{subfigure}
    \begin{subfigure}[b]{0.3\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Figures/pressure_and_shear_accumulation.png}
        \caption{Accumulated shear stress zoomed-in on roll-up point}
        \label{subfig:shear_pressure_accumulation_zoomed_in_on_rollup}
    \end{subfigure}
...
A Mesh sensitivity study was conducted based on the infiltration depth at 0.1 s of physical time. The results in Figure \ref{fig:meshSensResults} and Table \ref{tab:meshSensResults} show asymptotic convergence. With such incredibly small variations with coarser mesh sizes, a mesh with a base size of ${1.4x10^{-7}}$ m will be used for further simulations. Performing the formal grid convergence index (GCI) calculations \cite{ECA2014104, celik2008procedure} shows that the results in Table \ref{tab:meshSensResults} imply oscillatory convergence, because  $0<\epsilon_{21}/\epsilon_{32}=\ 0.004<1$, and a GCI of $7.5x10^{-4}$.
%The physical time to extract the infiltration depth was chosen because beyond that time, error accumulates in the low-fidelity simulation, where the mass of CMAS grows beyond the initial value, causing the result to be non-physical. This non-physical phenomenon can be seen in the coarsest mesh in Figure \ref{fig:mesh-a}, and the result becomes more clear in Figure \ref{fig:mesh-b} - \ref{fig:mesh-d}. 
...
\subsection{Results From open-pipe, concentric-pipe, planar Channel, and Feathery Channel Simulations}
...
The simulated results are compared to the analogous analytical pipe models, and experiments described in Naraparaju et. al. \cite{Naraparaju2019}. The formulations for the pipe models can be seen below. 
...
The results of the benchmark to the analytical models are shown in Figure \ref{fig:rect_v_feather}. Results from the open-pipe and concentric-pipe simulations are omitted because the simulated results contradict the analytical model. That is, when the simulation starts, the CMAS is already at it's equilibrium point. This is the case regardless of the initial depth, or of the properties of the CMAS, because the driving factor is the incredibly small radius of the tube, making the inertial response time,$t_{R} ~ \sqrt{\frac{\rho R^{3}}{\sigma}}$, of the flow on the order of $1\times 10^{-13} s$ \cite{Weislogel}. This of course, does not align with the outcomes of the open-pipe and concentric-pipe models \cite{Naraparaju2019}. This shows that the analytical open and concentric-pipe models are not capable of predicting the flow in one isolated channel without considering that channel's porosity to CMAS.

However, once porosity effects are considered in the form of geometry changes (i.e. the planar channel and the feathery channel), some meaningful results are captured. Figure \ref{fig:rect_v_feather} shows infiltration depth v. time for both the feathery and planar micro-channel.  Importantly, this verifies that the feathery micro-channel causes less infiltration than a planar channel with the same inter-columnar gap. Figure \ref{fig:rect_v_feather} also shows the analytical relations for the open and concentric-pipe models. Note that the non-constant viscosity is accounted for as the depth increases using the experimentally-measured viscosity data \cite{Naraparaju2019}, and the temperature gradient in Table \ref{tab:boundaryConditions}. This is not a direct comparison because porosity is handled differently in those models, but the results are at least of the same order of magnitude. It should be noted that the pipe models do not equillibrate, however, because of the basis in Darcy's Law. This provides evidence that data from resolving the flow in one TBC column cannot be extrapolated to the bulk flow in the porous network of columns. Further 3D evaluations are needed to compare to these analytical models. 

Figure \ref{fig:infilResult} shows the volume fraction of CMAS and the TBC temperature for simulations with and without a solidification model enabled, once the infiltration has reached equilibrium. In both cases, air pockets tend to form within the infiltrated CMAS. This supports the argument that the infiltrated CMAS can alter the thermal properties of the coating. Even at these high temperatures, the thermal conductivity of air is still two orders of magnitude smaller than that of the TBC and the CMAS, so the air pockets would certainly affect the conductivity of the resulting TBC/CMAS mixture. Figure \ref{fig:infilResult} also shows that the presense of a solidification model can change the final equilibrium depth, meaning that the overall infiltration kinetics may not be able to be resolved with viscous effects alone.
...
\begin{figure}
    ...
    \caption{Comparison of infiltration depth v. time for the planar micro-channel, and feathery micro-channel co-plotted with the expected results from analytical pipe models. The feathery channel equillibrates at a shorter depth and shorter time than the planar channel.}
    \label{fig:rect_v_feather}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{Figures/infilResults.png}
    \caption{Image of the feathery TBC simulation showing the volume fraction of CMAS and the temperature of the TBC. In the simulation that did not use a solidification model (left), it is easy to see that bubbles and air pockets tend to form within the CMAS. The image with a solidification model  (right) shows that the presence of a solidification model can cause the infiltration to stop at a smaller equilibrium depth. }
    \label{fig:infilResult}
\end{figure}
...
The effect of viscosity and surface tensions on the flow can be summarized using the Ohnesorge Number, ($Oh$). The planar and feathery micro-channel simulations were reevaluated, replacing the experimentally measured viscosity with a constant viscosity, while the surface tension was held constant. Thus, a range of $Oh$ were evaluated for both channels to understand how the ratio of viscous forces to inertial and surface tensions forces affects the infiltration time. Figure \ref{fig:rect_v_feather_oh} plots the non-dimensional equilibrium point (where $D^{*} = D/b$, $b$ is the columnar gap width) against infiltration depth for various $OH$. An interesting result is that, at lower $Oh$, the planar micro-channel is more effective at inhibiting infiltration, but around $Oh=500$, this trend switches, and the feathery microstructure becomes more effective.

\begin{figure}[htp!]
    \centering
    \includegraphics[width=\linewidth]{Figures/Rect_feather_compare.png}
    \caption{Non-dimensional equilibrium depth v. $Oh$ for the planar and feathery micro-channels. At lower $Oh$, the feathery microstructure allows the molten liquid to infiltrate further than the planar channel. However, around $Oh = 500$, this switches, and the feathery channel is more effective at stopping infiltration.}
    \label{fig:rect_v_feather_oh}
\end{figure}

\subsection{Characterization Based on Solidification}
The simulation was run with the experimental viscosity model, this time enabling the solidification model described in Figure \ref{sec:solidmodel}. Results in \ref{fig:solidification} show the infiltration in both the feathery and planar channel without the solidification model, and with the solidification model at varying levels of flowability threshold (the relative solid volume fraction in the cell where the flow will stop).

\begin{figure}[htp!]
    \centering
    \includegraphics[width=\linewidth]{Figures/solidificationStudy.png}
    \caption{Depth v. time for the feathery and planar micro-channels with solidification, turned off, and solidification turned on, with varying flowability threshold.}
    \label{fig:solidification}
\end{figure}

Clearly, the flowability threshold affects both the equilibrium depth and the time to equilibrate. However, no clear pattern is seen here. Low flowability threshold tends to produce flow similar to the flow without solidification enabled. Larger flowability thresholds produce more dissimilar results, but the equilibrium depth can either be lower or higher than the no solidification case. Some results also overlap perfectly, which suggests that after some threshold is crossed, the result will not change. These similar cases also take longer to equilibrate and equilibrate at further depths. Due to the inexplicable variability in these results, it is perhaps best to account for solidification via increasing viscosity rather than using an actual solidification model.
...
\section{CMAS/TBC Infiltration}
\label{sec:cmas_conc}
A simulation methodology to simulate CMAS infiltrating a 7YSZ EB-PVD TBC was formulated using the Eulerian VOF method. Results from this methodology show that the hypothesis that chemistry effects in infiltration are higher-order and can be ignored, is potentially true, due to agreement between experimental and simulated infiltration times. 

Another important result is that infiltration occurs on several different time scales, characterized by varying infiltration rates. When a CMAS particle first contacts the TBC column, it experiences a period of fast infiltration, likely dominated by the momentum of the particle. From that point onward, the particle experiences a capillary-driven infiltration that is much slower, both due to the capillary forces being very weak, and because the particle solidifies as it infiltrates, impeding the infiltration rate further.
